...
const DisplacementSphere = (props) => {
    ...
    useEffect(() => {
        ...
        try {
            ...
            console.warn('WebGL initialization failed, skipping 3D background');
            setWebGLSupported(false);
            return;
        }
    ...
    useEffect(() => {
        if (!webGLSupported || !scene.current) return;
    ...
    }, [rgbBackground, theme, webGLSupported]);

    useEffect(() => {
        if (!webGLSupported || !renderer.current) return;

        const handleResize = () => {
            if (!canvasRef.current || !renderer.current || !camera.current || !sphere.current || !scene.current) return;
    ...
    useEffect(() => {
        if (!webGLSupported || !sphere.current) return;
        ...
        return () => {
            ...
            if (sphereSpring.current) {
                sphereSpring.current = null;
            }
        };
    }, [isInViewport, prefersReducedMotion, webGLSupported]);
